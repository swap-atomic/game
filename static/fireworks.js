...
function Firework(oC,startX,startY,burstX,burstY,burstType,nRadius,nParticles,nCircles,allowRandom,obeyBoundaries) {
  ...
  //  - Don't slide the firework to position since you can't see that anyways... Just immediately blow up!
  /*
  self.moveTo(self.x,self.y);
  self.slideTo(burstX,burstY);
  */
  self.moveTo(burstX, burstY);
  self.aniExplode();
...
$(document).ready(function() {
    ...
    $(window).resize(fc.getWindowCoords);
    $(window).scroll(fc.getWindowCoords);

    fc.init();
    $(window).unload(fc.destructor);
});
